...
export class CustomersComponent implements OnInit,OnDestroy {
  ...
  get_all_users(){
   this.userList = []
    const sub = this.accountService.get_all_users().pipe(first()).subscribe((user: any) => {
  ...
  delete_user(id: any) {
    const confirmation = confirm('Are you sure you want to delete this user?' + id);
    if (confirmation) {
      const sub = this.accountService.deleteUser(id).pipe(first()).subscribe((user: any) => {
        console.log(user)
        if (user.success) {
